...
# Pinch — funktioniert mit BEIDEN Fingern egal auf welcher Seite
var _pinch_fingers: Dictionary = {}   # index → Vector2
...
func _handle_touch(event: InputEventScreenTouch, sw: float) -> void:
	...
	if event.pressed:
		# Pinch: sobald 2 Finger gleichzeitig aktiv sind
		_pinch_fingers[event.index] = pos
		if _pinch_fingers.size() == 2:
			var keys: Array = _pinch_fingers.keys()
			_pinch_last_dist = (_pinch_fingers[keys[0]] as Vector2).distance_to(
				_pinch_fingers[keys[1]] as Vector2
			)

		# Joystick: linke Hälfte
		if pos.x < sw * 0.5:
			if _js_finger < 0:
				_js_finger = event.index
				_js_origin = pos
				_update_js_visuals(pos, Vector2.ZERO)
		# Kamera: rechte Hälfte
		else:
			if _cam_finger < 0:
				_cam_finger = event.index
				_cam_last = pos
	else:
		_pinch_fingers.erase(event.index)
...
func _handle_drag(event: InputEventScreenDrag) -> void:
	# Pinch-Position aktualisieren
	if event.index in _pinch_fingers:
		_pinch_fingers[event.index] = event.position

	# Pinch-Zoom wenn 2 Finger aktiv
	if _pinch_fingers.size() == 2:
		var keys: Array = _pinch_fingers.keys()
		var new_dist: float = (_pinch_fingers[keys[0]] as Vector2).distance_to(
			_pinch_fingers[keys[1]] as Vector2
		)
		if _pinch_last_dist > 0.0:
			zoom_delta += (_pinch_last_dist - new_dist) * 0.025
		_pinch_last_dist = new_dist
		# Joystick weiter updaten aber kein Kamera-Swipe während Pinch
		if event.index == _js_finger:
