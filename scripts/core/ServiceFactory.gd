...
func instantiate_all(defs: Array[ServiceDefinition], registry: ServiceRegistry, parent: Node) -> bool:
	var ok := 0
	for def in defs:
...
func _from_scene(scene: PackedScene, def: ServiceDefinition, registry: ServiceRegistry, parent: Node) -> Node:
...
func _from_script(script: GDScript, def: ServiceDefinition, registry: ServiceRegistry, parent: Node) -> Object:
