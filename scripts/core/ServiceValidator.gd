...
func validate() -> Array[ServiceDefinition]:
	...
	for i in range(config.services.size()):
		...
		if def == null:
			Logger.log_error(
				"Eintrag am Index %d ist keine 'ServiceDefinition' (Klasse nicht erkannt?)" % i,
				LOG_CAT
			)
			errors += 1
			continue
	...
	if validation_errors > 0:
		Logger.log_error(
			"%d inhaltliche Fehler gefunden — Boot abgebrochen." % validation_errors, LOG_CAT
		)
		return []
...
func _check_definition(def: ServiceDefinition, index: int) -> int:
	...
	for res_path in def.required_data_files:
		if res_path.is_empty():
			Logger.log_warn(
				"Service %s: Leerer Pfad in 'required_data_files' gefunden." % identifier, LOG_CAT
			)
			continue
	...
	if def.service_name in def.deps:
		Logger.log_error(
			"Service %s: Abhängigkeit auf sich selbst gefunden (Circular)!" % identifier, LOG_CAT
		)
		errors += 1
