...
func setup_services(main_node: Node) -> void:
    ...
    _create("debug_console", main_node, "res://scripts/services/DebugConsole.gd")
...
func _create(service_name: String, parent: Node, path: String) -> void:
    ...
    parent.add_child(node)          # _ready() läuft NICHT sofort
    # Force _ready() sofort via notification:
    # Das geht nicht direkt – daher anderer Ansatz: siehe Main.gd