...
## Player.gd — Schlanke Version mit Service-Anbindung

# Wir laden Basis-Werte zentral aus dem DataService
@onready var speed: float = Data.PLAYER_BASE_STATS.get("speed", 6.0)
@onready var gravity: float = Data.PLAYER_BASE_STATS.get("gravity", 12.0)
@onready var interact_range: float = Data.PLAYER_BASE_STATS.get("interact_range", 4.0)

# Interne Variablen für Kamera-Smoothing
...
func _physics_process(delta: float) -> void:
	# 1. Status-Check: Wenn beschäftigt, direkt raus
	if not States.is_free(): 
		# Wir setzen die Geschwindigkeit auf 0, damit der Player nicht "gleitet"
		velocity.x = 0
		velocity.z = 0
		move_and_slide()
		return
...
func _handle_camera(touch: Node, delta: float) -> void:
	# Delegation 2: Kamera-Werte verrechnen
...
func _handle_movement(touch: Node, delta: float) -> void:
	...
	if input.length() > 0.1:
		# Delegation 3: Utils berechnet die Richtung relativ zur Kamera
		var move_dir = Utils.calculate_move_direction(_spring_arm, input)
...
func try_interact() -> void:
	# Delegation 4: Utils sucht das nächste interaktive Objekt
	var target = Utils.get_closest_node(global_position, "interactable", interact_range)
	if target:
...
func _build_player_nodes() -> void:
	# (Kollision, Mesh, SpringArm wie gehabt)
