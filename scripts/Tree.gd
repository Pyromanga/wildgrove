extends Node3D
## Tree.gd — Vollständig mit Krone & Respawn
...
const TREE_DATA: Dictionary = {
	"normal": { "id": "log_normal", "xp": 25, "req": 1,  "time": 3.0, "color": Color(0.1, 0.4, 0.1) },
	"oak":    { "id": "log_oak",    "xp": 45, "req": 15, "time": 5.0, "color": Color(0.05, 0.3, 0.05) }
}
...
func _on_chopped(_src, _res) -> void:
	...
	if ss and ss.meets_requirement("woodcutting", _data["req"]):
