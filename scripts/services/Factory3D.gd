...
class Bar3D extends Node3D:
    ...
    func update(percent: float) -> void:
        # FIX: Expliziter Typ float statt :=
...
func create_3d_bar(parent: Node3D) -> Bar3D:
    ...
    var bg: MeshInstance3D = _create_3d_shape(QuadMesh.new(), Color.BLACK)
    # Cast auf QuadMesh, um auf .size zuzugreifen (sicherer für Compiler)
    if bg.mesh is QuadMesh:
        bg.mesh.size = Vector2(1.2, 0.2)
    root.add_child(bg)

    var fill: MeshInstance3D = _create_3d_shape(QuadMesh.new(), Color.YELLOW)
...
func create_simple_tree(parent: Node3D) -> Node3D:
    ...
    var trunk: MeshInstance3D = _create_3d_shape(CylinderMesh.new(), TREE_TRUNK_COLOR)
    trunk.scale = Vector3(0.3, 1.5, 0.3)
    trunk.position.y = 0.75
    root.add_child(trunk)

    var leaves: MeshInstance3D = _create_3d_shape(SphereMesh.new(), TREE_LEAVES_COLOR)
