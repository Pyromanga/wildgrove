...
func transition_to_state(state: GameEnums.State) -> void:
    ...
    if path:
        Logger.log_info("Wechsle Szene für State %s: %s" % [GameEnums.State.keys()[state], path], LOG_CAT)
        get_tree().call_deferred("change_scene_to_file", path)
    else:
        Logger.log_debug("Kein Szenenwechsel für State: %s" % GameEnums.State.keys()[state], LOG_CAT)
